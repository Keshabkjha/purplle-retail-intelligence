...
def test_store_metrics_calculations(client, db_session):
    ...
    db_session.add(DBEvent(
        event_id="e1", store_id=store_id, camera_id="CAM_ENTRY_01", visitor_id=vid_cust,
        event_type="ENTRY", timestamp="2026-04-10T10:00:00Z", zone_id="ENTRY",
        dwell_ms=0, is_staff=False, confidence=0.9, metadata_json={"session_seq": 1}
    ))
    # Enter retail zone
    db_session.add(DBEvent(
        event_id="e2", store_id=store_id, camera_id="CAM_MAIN_01", visitor_id=vid_cust,
        event_type="ZONE_ENTER", timestamp="2026-04-10T10:01:00Z", zone_id="EB_KOREAN",
        dwell_ms=0, is_staff=False, confidence=0.9, metadata_json={"session_seq": 2}
    ))
    # Billing queue join
    db_session.add(DBEvent(
        event_id="e3", store_id=store_id, camera_id="CAM_BILLING_01", visitor_id=vid_cust,
        event_type="BILLING_QUEUE_JOIN", timestamp="2026-04-10T10:05:00Z", zone_id="BILLING",
        dwell_ms=60000, is_staff=False, confidence=0.9, metadata_json={"session_seq": 3, "queue_depth": 1}
    ))
    # EXIT
    db_session.add(DBEvent(
        event_id="e4", store_id=store_id, camera_id="CAM_ENTRY_01", visitor_id=vid_cust,
        event_type="EXIT", timestamp="2026-04-10T10:10:00Z", zone_id="ENTRY",
        dwell_ms=0, is_staff=False, confidence=0.9, metadata_json={"session_seq": 4}
    ))

    # Add staff events (should be excluded)
    db_session.add(DBEvent(
        event_id="s1", store_id=store_id, camera_id="CAM_ENTRY_01", visitor_id=vid_staff,
        event_type="ENTRY", timestamp="2026-04-10T10:00:00Z", zone_id="ENTRY",
        dwell_ms=0, is_staff=True, confidence=0.9, metadata_json={"session_seq": 1}
    ))
    ...
    assert metrics["conversion_rate"] == 100.0  # Converted since billing was 10:05:00 and purchase was 10:08:00 (within 5 mins)
...
def test_funnel_and_heatmap_values(client, db_session):
    ...
    # ENTRY
    db_session.add(DBEvent(
        event_id="f1", store_id=store_id, camera_id="CAM_ENTRY_01", visitor_id=vid,
        event_type="ENTRY", timestamp="2026-04-10T11:00:00Z", zone_id="ENTRY",
        dwell_ms=0, is_staff=False, confidence=0.95, metadata_json={"session_seq": 1}
    ))
    # Retails Zone visit
    db_session.add(DBEvent(
        event_id="f2", store_id=store_id, camera_id="CAM_MAIN_01", visitor_id=vid,
        event_type="ZONE_ENTER", timestamp="2026-04-10T11:02:00Z", zone_id="EB_KOREAN",
        dwell_ms=30000, is_staff=False, confidence=0.95, metadata_json={"session_seq": 2}
    ))
    db_session.commit()
