...
def test_supervised_artifacts_load_at_runtime(tmp_path, monkeypatch):
    ...
    assert registry.predict_staff_probability(
        staff_features, fallback=0.0
    ) > registry.staff_model.predict_proba(staff_features, fallback=0.0)
    assert registry.predict_identity_probability(
        identity_features, fallback=0.0
    ) > registry.identity_model.predict_proba(identity_features, fallback=0.0)
