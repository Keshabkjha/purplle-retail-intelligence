...
def _extract_label(record: dict) -> int:
    ...
    raise ValueError("Training record must include a label field such as label/is_staff/match/same_person")


def _extract_feature_array(record: dict, feature_builder, required_keys: tuple[str, ...]) -> np.ndarray:
...
def train_binary_classifier(X: Sequence[Sequence[float]], y: Sequence[int]):
    ...
    if len(np.unique(y_arr)) < 2:
        raise ValueError("Need at least one positive and one negative example to train a binary classifier")
    ...
    for record in records:
        features.append(
            _extract_feature_array(
                ...
                ("torso_match_ratio", "is_clothing_staff", "zone_id", "wx", "billing_duration_sec", "total_duration_sec", "camera_count"),
            )
        )
    ...
    for record in records:
        features.append(
            _extract_feature_array(
                ...
                ("spatial_score", "temporal_score", "visual_score", "camera_score", "zone_score", "dist_norm", "time_norm"),
            )
        )
        labels.append(_extract_label(record))
...
class AdaptiveModelRegistry:
    ...
    def __post_init__(self):
        ...
        self.identity_model = AdaptiveBinaryModel(os.path.join(self.base_dir, "identity_online.pkl"))
        self.supervised_staff_model = StaticBinaryModel(
            os.getenv("STAFF_SUPERVISED_MODEL_PATH", os.path.join(self.base_dir, "staff_supervised.pkl"))
        )
        self.supervised_identity_model = StaticBinaryModel(
            os.getenv("IDENTITY_SUPERVISED_MODEL_PATH", os.path.join(self.base_dir, "identity_supervised.pkl"))
        )
