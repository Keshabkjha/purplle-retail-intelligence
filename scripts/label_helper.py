...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Create tiny JSONL annotations for supervised training"
    )
...
def build_record(mode: str) -> dict:
    if mode == "staff":
        return {
            "torso_match_ratio": prompt_float(
                "torso_match_ratio", STAFF_TEMPLATE["torso_match_ratio"]
            ),
            "is_clothing_staff": prompt_bool(
                "is_clothing_staff", STAFF_TEMPLATE["is_clothing_staff"]
            ),
            "zone_id": prompt_str("zone_id", STAFF_TEMPLATE["zone_id"]),
            "wx": prompt_float("wx", STAFF_TEMPLATE["wx"]),
            "billing_duration_sec": prompt_float(
                "billing_duration_sec", STAFF_TEMPLATE["billing_duration_sec"]
            ),
            "total_duration_sec": prompt_float(
                "total_duration_sec", STAFF_TEMPLATE["total_duration_sec"]
            ),
